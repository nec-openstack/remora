...
if [[ ${REMORA_DEFAULT_ENV_IS_LOADED:-""} != "true" ]]; then
    ...
    export KUBE_ADDITIONAL_HOSTNAMES=${KUBE_ADDITIONAL_HOSTNAMES:-""}
    export KUBE_ADDITIONAL_SERVICE_IPS=${KUBE_ADDITIONAL_SERVICE_IPS:-""}
    export KUBE_DNS_SERVER_IP=${KUBE_DNS_SERVER_IP:-"10.254.0.10"}
    export KUBE_NETWORK_PLUGIN=${KUBE_NETWORK_PLUGIN:-"cni"}
    ...
    export REMORA_DEFAULT_ENV_IS_LOADED="true"
